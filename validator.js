...
Validator.validate = function(validators, singlerule) {
		this.hasErrors = false;
		...
		if(singlerule === undefined) {
				...
				if(validators[singlerule] !== undefined) {
						if (validators[singlerule].validator === undefined || (validators[singlerule].validator !== true && validators[singlerule].validator !== false)) {
								throw "you must specify a validator key in a validator object, the value of this key Must be a function";
						}
						if (validators[singlerule].onFail === undefined || (validators[singlerule].onFail instanceof Function === false)) {
								throw "you must specify a onFail key in a validator object, the value of this key Must be a function";
						}
						if (validators[singlerule].validator === true) {
								validators[singlerule].onSuccess();
						} else {
								this.hasErrors = true;
								validators[singlerule].onFail();
						}
				} else {
						throw "you must specify a matching singlerule key from your validators object to validate";
				}
		}

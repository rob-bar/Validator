...
Validator.validateLength = function(str, min, max) {
    if (min == 0 && max == 0) throw "Validator : When calling validateLength min or max should be different from 0.";
    if ((min >= max || max <= min) && (max != 0)) throw "Validator : When calling validateLength min is the minimum and max should be the maximum.";
    if (min != 0 && max != 0) {
...
Validator.validateRange = function(value, min, max) {
    ...
    (min === undefined) ? min = 0: min = min;
    if ((min >= max || max <= min) && (max != 0)) throw "Validator : When calling validateRange min is the minimum and max should be the maximum.";
    var ivalue = value;
    if (min != 0 && max != 0) {
...
Validator.validateIsequalto = function(str, comparrer, ignoreWhitespaceInbackandfront) {
    ...
    if (ignoreWhitespaceInbackandfront == true) {
        ...
        comparrer = StringUtil.trim(comparrer);
        str = StringUtil.rtrim(str);
        comparrer = StringUtil.rtrim(comparrer);
    }
    if (str == comparrer) {
        return true;
    } else {
